...
function Debit() {
  ...
  const [debiterName, setDebiterName] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    const forCreatingDebit = await firebase.handleNewDebit(
      debiterName,
      leftAmount,
      0,
      firebase.user.uid
  ...
  return (
    <div className="bg-slate-900 h-screen">
      ...
      >
        <h2 className="text-white mb-4">Add Debit</h2>
        <Form className="columns-md flex-col space-y-3 ">
          <div className="relative">
            <input
              ...
              placeholder=" "
              onChange={(e) => setDebiterName(e.target.value)}
            />
