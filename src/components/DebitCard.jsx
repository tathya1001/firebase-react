...
const DebitCard = (props) => {
    ...
    const handleAddConfirm = () => {
        if (additionInput.trim() !== "") {
            firebase.handleCreditAdd(props.id, props.left, parseInt(additionInput), firebase.user.uid);
    ...
    const handlePayConfirm = () => {
        if (paymentInput.trim() !== "") {
            firebase.handleCreditPay(props.id, props.completed, parseInt(paymentInput), firebase.user.uid);
    ...
    return (
        ...
        <div className="h-24 bg-[#4F060E] flex px-3 py-2 rounded-[1rem] gap-2 font-[Rubik] box-border">
            ...
            {isAddPopupOpen && (
                <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center bg-gray-900 bg-opacity-50">
                    <div className="bg-white p-4 rounded-md">
                        <input type="number" value={additionInput} onChange={(e) => setAdditionInput(e.target.value)} className="w-full text-white border-gray-300 rounded-md p-2 focus:outline-none" />
                        <div className="flex justify-between mt-4">
                            <button className="bg-red-500 text-white px-4 py-2 rounded-md mr-2" onClick={handleAddCancel}>Cancel</button>
                            <button className="bg-green-500 text-white px-4 py-2 rounded-md" onClick={handleAddConfirm}>Confirm</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {isPayPopupOpen && (
                <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center bg-gray-900 bg-opacity-50">
                    <div className="bg-white p-4 rounded-md">
                        <input type="number" value={paymentInput} onChange={(e) => setPaymentInput(e.target.value)} className="w-full text-white border-gray-300 rounded-md p-2 focus:outline-none" />
                        <div className="flex justify-between mt-4">
                            <button className="bg-red-500 text-white px-4 py-2 rounded-md mr-2" onClick={handlePayCancel}>Cancel</button>
                            <button className="bg-green-500 text-white px-4 py-2 rounded-md" onClick={handlePayConfirm}>Confirm</button>
                        </div>
                    </div>
                </div>
            )}
